...
#map------------------------------------------------------------------------------------
  def create_map_link(zoom=16,width=800,height=800)
     markers = ""; i = 1
      markers += "&markers=color:red%7Ccolor:red%7Clabel:#{i}%7C#{self.location.latitude},#{self.location.longitude}"
      i += 1
      map = "http://maps.google.com/maps/api/staticmap?center= #{self.location.latitude},#{self.location.longitude}&zoom=#{zoom}&size=#{width}x#{height}&maptype=roadmap#{markers}&sensor=false"
  end
end
